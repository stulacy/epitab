...
```{r}
treat <- data.frame(age=rnorm(100, 60, 20),
                    sex=factor(sample(c("M", "F"), 100, replace=T), levels=c('F', 'M')),
                    treated=factor(sample(c("Yes", "No"), 100, replace=T), levels=c('Yes', 'No')),
                    athome=factor(sample(c("Yes", "No"), 100, replace=T), levels=c('Yes', 'No')))
treat$agebin <- cut(treat$age, breaks=c(0, 20, 40, 60, 80, 9999),labels=c("0-20", "20-40", "40-60", "60-80", "80+")) 
```
...
```{r}
contingency_table(list("Age at diagnosis"="agebin",
                       "Sex"="sex"),
                  data=treat,
...
contingency_table(list("Age at diagnosis"="agebin",
                  ...
                  data=treat, 
                  outcomes=list("Treated"='treated'),
                  row_funcs=list("Adjusted odds ratio"=odds_ratio('treated', adjusted = TRUE)))
```
...
contingency_table(list("Age at Diagnosis (years)"="agegrp",
                  ...
                  data=prevsim,
                  frequency = T,
                  outcomes=list("Five year survival"="fiveyrsurv"),
                  row_funcs=list("Hazard ratio"=hazard_ratio("Surv(time, status)"),
                              "Adjusted hazard ratio"=hazard_ratio("Surv(time, status)", adjusted=TRUE))
                  )
```
...
TODO Talk about `neat_table` here
...
TODO Discuss that can use kable on raw matrix for more fine tuned control
...
knitr::kable(contingency_table(list("Age at diagnosis"="age",
                               ...
                               outcomes=list("Treated"='treated'),
                               row_funcs=list("Odds ratio"=odds_ratio('treated'),
                                           "Adjusted odds ratio"=odds_ratio('treated', adjusted=TRUE)))$mat,
             "html"
)
```
...
For the advanced user, any regression model can be provided to the `model` argument, and not just the specified options (`odds_ratio`, `adj_odds_ratio`, `hazard_ratio`, `adj_hazard_ratio`). These must be specified as functions with two parameters: 
