...
`epitab` provides functionality for building contingency tables with options for other summary measures built in. It was initially designed for use in epidemiology, as an extension to the `Epi::stat.table` function, however, by using a generalised framework of a descriptive table, it is flexible enough to be used in a variety of disciplines and situations. This vignette provides an overview of the types of tables that can be built using `epitab`.

## Data

For demonstration purposes, a simulated data set representing an observational study of a disease will be used. The disease mostly affects elderly people, and not every patient receives firstline treatment. The disease itself comes in two variants: *A* and *B*.
...
Contingency tables are useful when running an initial exploratory analysis of a data set, and highlight relationships between one or more *independent* variables and (typically one) outcome. The code example below shows how to build a basic contingency table to see how treatment varies by age and sex.

Both the *independent* and *outcome* variables are passed in through lists, with the name providing the column or row label. The column names **must** be quoted strings, allowing for these tables to be used in automated scripts. The `crosstab_funcs` argument specifies what summary measures should be calculated for each independent variable / outcome combination. The `freq` function calculates the frequency of each of these cells, and (optionally) provides the proportion in parantheses.
...
Note that multiple outputs can be included, although these are still 2-way contingency tables between all the covariates and the outcomes independently. It is not currently possible to produce a 3-way contingency table.
...
Supplementary data and statistics can be added to these standard contingency tables in two ways. *column-wise measures* act on each outcome in turn, without regard to the covariates, while *row-wise measures* are those that are calculated for every level of every independent variable.

### Column-wise measures

It is often the case that in addition to the categorical variables included in the contingency table, there are continuous attibutes that we are interested in. The `col_funcs` argument to `contingency_table` calculates summary measures for each outcome and can be used for this purpose.

The example below shows how to calculate mean age across treatment types, using the built-in function `summary_mean`, to which the continuous variable of interest is passed as a string.
...
Another common addition is to display the coefficients of a regression regression model that relates the independent variables with the outcome. For example, we may be interested to see how treatment varies by age by looking at the odds ratios (ORs) of a univariate logistic regression. This behaviour is specified by the `row_funcs` argument to `contingency_table`, which accepts a named list of functions that meet the correct requirements. The two functions provided with this package are `odds_ratio` and `hazard_ratio`, used to display coefficients resulting from logistic regression and Cox regression respectively. 
...
Another use case in epidemiology is where survival is the outcome of interest. Such data is more appropriately modelled using Cox regression, which can be specified with the `hazard_ratio` function. This requires the outcome to be specified as a string detailing a `Surv` object, for example `hazard_ratio("Surv(time, status)")`. The help page provides further details.
...
![`neat_table()` LaTeX example](neat_latex_resize.png)

### `kable`

If full control of the table appearence is required, then the raw character matrix is provided as the `mat` attribute of the output of `contingency_table`. It can be used in conjunction with the fantastic `knitr::kable` function, and the `kableExtra` package also provides excellent options for tidy tables. **Although again note that these changes will not display in the vignette you are currently reading due to the template, but they will appear in your markdowns**.

NB: The default value for `format` in `kable` is *pandoc*, which does not work well with `epitab`, try *html* or *markdown* instead.
...
As Word is a proprietary format, it is a bit directly embed tables into documents. The most convenient way involves a few steps:

  1. Save the raw table as a CSV
...
The code snippet below shows how to save the raw table to CSV.
...
In the above examples, the summary functions used to build up the table in `crosstab_funcs`, `row_funcs`, and `col_funcs` have been provided by `epitab`. However, for greater flexibility, any correctly-parameterised function can be used. This section details the appropriate form for each of these 3 arguments.
...
Args:
  ...
  - `outlevel`: The current level of the outcome, as a string.
  - `outcome`: The current outcome, as a string.
...
The example function below extends `summary_mean` by adding the standard deviation in parantheses. It is hardcoded to work for the continuous variable `age` in the dummy data set.
...
Row-wise functions are used to estimate summary measures for the independent variables outside of the contingency table. This can be useful for providing a summary statistic that is not necessarily related to the outcome variables.

This example will describe the case where we wish to run a linear regression on a continuous outcome using the independent variables, in particular, we will be running a linear regression on continuous age. This is not a particularly helpful analysis, particularly since one of the covariates is age group, but it will serve as an example. These functions must be parameterised as follows:

Args:

  - `var`: The current independent variable, as a string.
  - `all_vars`: All independent variables as passed into `independents`.
